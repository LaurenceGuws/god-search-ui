...
| `scripts/check_shell_health_contract.sh` | Shell health fallback/live runtime contract checks | `scripts/check_shell_health_contract.sh` | (same command) |
| `scripts/control_plane_smoke.sh` | Control-plane command-matrix runtime smoke | `scripts/control_plane_smoke.sh` | (same command) |
| `scripts/wm_event_refresh_smoke.sh` | WM event refresh runtime smoke (Hyprland event bridge path) | `scripts/wm_event_refresh_smoke.sh` | (same command) |
