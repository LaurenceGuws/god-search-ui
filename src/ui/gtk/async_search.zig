...
const max_async_rows: usize = 200;
...
fn asyncRouteSearchWorker(payload: *AsyncSearchResult) void {
    ...
    if (payload.total_len > limit) {
        std.log.warn("async search truncated query={s} total={d} stored={d}", .{ payload.query, payload.total_len, limit });
    }
