...
SOCKET_PATH="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/wayspot.sock"
...
Modes:
  takeover  Stop swaync, start wayspot --ui-daemon, verify bus owner.
  restore   Kill wayspot daemon and restart swaync service/socket.
...
case "$MODE" in
    takeover)
        ...
        if "$BIN" --ctl ping >/dev/null 2>&1; then
            echo "[dev-notify] wayspot daemon already running"
        else
            echo "[dev-notify] starting wayspot --ui-daemon"
            "$BIN" --ui-daemon >/tmp/wayspot-dev-notify.log 2>&1 &
            sleep 0.8
        fi

        if ! "$BIN" --ctl ping >/dev/null 2>&1; then
            echo "[dev-notify] ERROR: control ping failed"
            echo "[dev-notify] tail /tmp/wayspot-dev-notify.log"
            tail -n 50 /tmp/wayspot-dev-notify.log 2>/dev/null || true
            exit 1
        fi
        ...
        if [[ "$INFO" != *"wayspot"* ]]; then
            echo "[dev-notify] WARNING: bus owner is not wayspot"
            exit 2
        fi
        ...
        ;;
    restore)
        echo "[dev-notify] stopping wayspot daemon"
        pkill -x wayspot 2>/dev/null || true
    ...
    smoke)
        ...
        INFO="$(bus_info)"
        if [[ "$INFO" != *"wayspot"* ]]; then
            echo "[dev-notify] ERROR: org.freedesktop.Notifications is not owned by wayspot"
        ...
        ID1_RAW="$(notify-send -p "wayspot smoke" "replace-one" || true)"
        ID1="$(extract_uint "$ID1_RAW")"
        require_uint "$ID1" "replace first"

        ID2_RAW="$(notify-send -p -r "$ID1" "wayspot smoke" "replace-two" || true)"
        ...
        ACTION_OUT="$(gdbus call --session \
            ...
            app 0 '' 'wayspot smoke action' 'click action button to emit ActionInvoked' \
            "['default','Open']" "{}" 5000)"
        ...
        timeout 8 dbus-monitor "interface='org.freedesktop.Notifications'" >/tmp/wayspot-notify-smoke.log 2>&1 &
        MON_PID=$!
        sleep 0.4
        gdbus call --session \
            --dest org.freedesktop.Notifications \
        ...
        PERSIST_RAW="$(notify-send -p -a "wayspot" -u low -t 0 "persistent test" "Should stay until dismissed" || true)"
        ...
        notify-send -t 900 "wayspot smoke" "timeout-check"
        sleep 2.2
        kill "$MON_PID" 2>/dev/null || true
        wait "$MON_PID" 2>/dev/null || true

        echo "[dev-notify] signal extract:"
        rg -n "member=NotificationClosed|uint32 [0-9]+" /tmp/wayspot-notify-smoke.log | tail -n 10 || true
        rg -q "uint32 3" /tmp/wayspot-notify-smoke.log || { echo "[dev-notify] ERROR: missing reason=3 close signal"; exit 5; }
        rg -q "uint32 1" /tmp/wayspot-notify-smoke.log || { echo "[dev-notify] ERROR: missing reason=1 timeout signal"; exit 6; }
